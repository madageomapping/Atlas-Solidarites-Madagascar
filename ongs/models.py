...
class mdgShoreline(gismodels.Model):
    ...
    class Meta:
        verbose_name_plural = "Littoraux"

    def __unicode__(self):
        return u"Pays: %s" % self.name

class mdgRegion(gismodels.Model):
    id_faritan = models.IntegerField()
    faritany = models.CharField(max_length=75)
...
class mdgDistrict(gismodels.Model):
    ...
    id_region = models.IntegerField()
    region = models.CharField(max_length=75)
    id_distric = models.IntegerField()
    ...
    def __unicode__(self):
        return u"District: %s" % self.district

    # Retourne un rapide descriptif
    def short(self):
        return u"District de %s \n de la région de %s" % (self.district, self.region)
    short.allow_tags = True

class mdgCommune(gismodels.Model):
    id_faritan = models.IntegerField()
    faritany = models.CharField(max_length=75)
    id_region = models.IntegerField()
    region = models.CharField(max_length=75)
    ...
    # Retourne la chaîne de caractère définissant le modèle.
    def __unicode__(self):
        return u"Commune: %s" % self.commune

    # Retourne un rapide descriptif
    def short(self):
        return u"Commune %s \n du district de %s" % (self.commune, self.district)
    short.allow_tags = True
